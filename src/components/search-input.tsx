...
export const SearchInput = ({
    ...
    return (
        <div className="relative w-full">
            ...
            <input type="text" placeholder="Search" className={`w-full ${backgroundColor} ${borderColor} outline-none rounded-[8px] ${theme === 'dark' ? 'text-white/20 placeholder:text-white/20' : 'text-black/20 placeholder:text-black/20'} focus:outline-none focus:ring-0 active:outline-none active:ring-0 pr-2 pl-7 py-1 text-sm`} value={value} onChange={(e) => onChange(e.target.value)} />
            {showIcon && (
                <>
