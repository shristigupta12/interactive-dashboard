    ...
    return(
        <div className={`w-full h-full rounded-[16px] ${theme === 'dark' ? detail.darkModeColor : detail.color} p-4 sm:p-6 flex flex-col gap-2 `}>
            ...
            <div className="flex items-center justify-between">
                <p className={`text-md sm:text-3xl font-semibold ${theme === 'dark' ? detail.darkModeTextColor : detail.textColor}`}>{detail.value}</p>
                <div className="flex items-center gap-1 rounded-[8px]">
                    <p className={`text-xs font-[300] ${theme === 'dark' ? detail.darkModeTextColor : detail.textColor}`}>{detail.changeType==="positive" ? "+" : detail.changeType==="negative" ? "-" : ""} {detail.percentageChange}</p>
